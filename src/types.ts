...
export interface FilesToolEntry<TUI extends Record<string, string> = Record<string, string>> {
  id: string;
  icons: {
    bg: string;
    fg: string;
...
export interface FilesCategoryEntry {
  icon: string;
  tools: FilesToolEntry[];
  i18n: LocaleMap<CategoryLocaleContent>;
}

export interface ToolDefinition {
  entry: FilesToolEntry;
  Component: unknown;
