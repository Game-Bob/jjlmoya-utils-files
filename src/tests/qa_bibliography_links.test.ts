...
function validateEntry(tool: string, entry: { url: string }, seen: Set<string>): LinkFailure[] {
  let url: URL;
  try {
...
async function validateTool(tool: (typeof ALL_TOOLS)[number]): Promise<LinkFailure[]> {
  ...
  return (content.bibliography ?? []).flatMap((entry) => validateEntry(tool.entry.id, entry, seen));
}
